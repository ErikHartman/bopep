...
if __name__ == "__main__":
    ...
    #pca_fit_peptides = pd.read_csv('/home/er8813ha/docking-peptide/data/2_filtered/peptide_data_filtered.csv')["peptide"].tolist()
    # Load proteome
    ...
    #remove if contains Immunoglobulin
    proteome = {k: v for k, v in proteome.items() if "immunoglobulin" not in k.lower()}
    logging.info(f"Filtered proteome to {len(proteome)} proteins after removing Immunoglobulins")
    ...
    DOCKING_KWARGS = {
        ...
        "save_raw": False,
        "force": False,
        "output_dir": "/srv/data1/er8813ha/bopep/proteome_docked/cd14_proteome_search",
    }
    ...
    ps = ProteomeSearch(
        ...
        # Initial sampling
        n_init=500,  # Start with 500 random peptides from proteome
        
        # Search parameters
        k_propose=50000,  # Sample 50k peptides from proteome each iteration
        ...
        # Logging
        log_dir="/home/er8813ha/docking-peptide/results_proteome/",
    )
    ...
    logging.info(f"Peptide length range: 8-35")
